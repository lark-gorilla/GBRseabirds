...
# core areas
for(i in c("BRBO", 'MABO', 'RFBO', 'FRBD', 'TRBD', 'WTST', "WTLG", "SOTE" , 'NODD', 'TERN'))
{
if(i=='BRBO'){s1<-read_sf(paste0('C:/seabirds/data/GIS/col_radii_core_hotspots_', i, '.shp'))
s1<-s1[,-1]}else{
  s2<-read_sf(paste0('C:/seabirds/data/GIS/col_radii_core_hotspots_', i, '.shp'))
  s2<-s2[,-1]
  s1<-rbind(s1,s2)}
}
s1<-s1%>%group_by(md_spgr)%>%st_buffer(0)%>%summarize(geometry = st_union(geometry))
...
pal <- colorBin(rev(c('#9e0142','#d53e4f','#f46d43','#fdae61','#fee08b','#e6f598','#abdda4','#66c2a5','#3288bd','#5e4fa2')), bins=10, c(NA, seq(0, 1, 0.1)), na.color = "transparent")
 ...
 m <- leaflet() %>%
  ...
  addRasterImage(brbo_ras, colors = pal, opacity = 0.8, group='BRBO')%>%
   ...
   addPolygons(data=filter(rad_diss, md_spgr=='BRBO'), fill=F,group='BRBO', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='MABO'), fill=F, group='MABO', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='RFBO'), fill=F, group='RFBO', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='FRBD'), fill=F, group='FRBD', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='TRBD'), fill=F, group='TRBD', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='WTST'), fill=F, group='WTST', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='WTLG'), fill=F, group='WTLG', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='SOTE'), fill=F, group='SOTE', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='NODD'), fill=F, group='NODD', weight=3, color='black')%>%
   addPolygons(data=filter(rad_diss, md_spgr=='TERN'), fill=F, group='TERN', weight=3, color='black')%>%
   
   addPolygons(data=filter(s1, md_spgr=='BRBO'), fill=F, color='red', weight=3, group='BRBO')%>%
   addPolygons(data=filter(s1, md_spgr=='MABO'),  fill=F, color='red', weight=3, group='MABO')%>%
   addPolygons(data=filter(s1, md_spgr=='RFBO'), fill=F, color='red', weight=3, group='RFBO')%>%
   addPolygons(data=filter(s1, md_spgr=='FRBD'), fill=F, color='red', weight=3, group='FRBD')%>%
   addPolygons(data=filter(s1, md_spgr=='TRBD'), fill=F, color='red', weight=3, group='TRBD')%>%
   addPolygons(data=filter(s1, md_spgr=='WTST'), fill=F, color='red', weight=3, group='WTST')%>%
   addPolygons(data=filter(s1, md_spgr=='WTLG'), fill=F, color='red', weight=3, group='WTLG')%>%
   addPolygons(data=filter(s1, md_spgr=='SOTE'), fill=F, color='red', weight=3, group='SOTE')%>%
   addPolygons(data=filter(s1, md_spgr=='NODD'), fill=F, color='red', weight=3, group='NODD')%>%
   addPolygons(data=filter(s1, md_spgr=='TERN'), fill=F, color='red', weight=3, group='TERN')%>%
